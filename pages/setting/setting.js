...
Page({
  ...
  data: {
    ...
    api: [
      ...
      { iconurl: '/images/icon/wx_app_list.png', title: '用户登陆', tap: 'login' },
      { iconurl: '/images/icon/wx_app_list.png', title: '校验用户信息', tap: 'check' },
      { iconurl: '/images/icon/wx_app_list.png', title: '获取用户加密信息', tap: 'decrypted' },
      { iconurl: '/images/icon/wx_app_list.png', title: '模板消息', tap: 'tplMessage' },
      { iconurl: '/images/icon/wx_app_list.png', title: '微信支付', tap: 'wxPay' }
    ],
    others: [
      { iconurl: '/images/icon/wx_app_list.png', title: 'wx:key示例', tap: 'showWxKeyDemo' },
      { iconurl: '/images/icon/wx_app_list.png', title: 'scroll-view高级用法演示', tap: 'showScrollViewDemo' }
  ...
  //清除缓存
  clearCache: function () {
    this.showModal("提示", "你确定要清理缓存吗？", function () {
      wx.clearStorage({
  ...
  showNetWork: function () {
    ...
    wx.getNetworkType({
      success: function (res) {
        ...
        that.showModal("网络状态", "您的当前网络状态是：" + networkType);
      },
    });
  },
  ...
  //在地图上显示当前位置
  showMap: function () {
    this.getLonLat(function (lon, lat, speed, accuracy) {
  ...
  getLonLat: function (callback) {
    ...
    wx.getLocation({
      ...
      success: function (res) {
        console.log(res)
        callback(res.longitude, res.latitude, res.speed, res.accuracy);
      }
    });
  },
  //显示当前位置坐标与当前速度
  showLonLat: function () {
    ...
    this.getLonLat(function (lon, lat, speed, accuracy) {
      ...
      that.showModal('当前位置和速度', '当前位置：' + lonStr + ',' + latStr + '。速度:' + speed + 'm/s' + " 精确度：" + accuracy);
    });
  },
  //显示罗盘
